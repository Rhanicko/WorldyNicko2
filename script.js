...
class WordlyKidsApp {
  ...
  init() {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      this.setupPoemModalInteractions()
      this.setupPoemSearch()
      this.setupPoemFilters()
  ...
  createThemeToggle() {
    ...
    // Find the best place to add the theme toggle
  ...
  navigateToStory(card) {
    ...
    // // Fix the path - if we're in html/ directory, don't add it again
    // if (window.location.pathname.includes("/html/") && !targetURL.includes("/html/")) {
    //   // We're already in html directory, use relative path
    //   targetURL = targetURL
    // } else if (!window.location.pathname.includes("/html/") && !targetURL.includes("/html/")) {
    //   // We're in root, need to add html/ prefix
  ...
  // Enhanced Poem Modal Interactions
  setupPoemModalInteractions() {
    const poemCards = document.querySelectorAll(".poem-card")
    let poemModal = document.getElementById("poem-modal")

    // Create modal if it doesn't exist
    if (!poemModal) {
      this.createPoemModal()
      poemModal = document.getElementById("poem-modal")
    }

    // Set up poem card interactions
  ...
  createPoemModal() {
    ...
    modal.id = "poem-modal"
    modal.className = "poem-modal hidden"
    ...
    <div class="modal-content">
      <button class="modal-close" aria-label="Close poem">&times;</button>
      <div class="modal-header">
        <h2 class="modal-title" id="modal-title"></h2>
        <p class="modal-author"></p>
      </div>
      <div class="modal-body">
        <div class="modal-poem-content"></div>
      </div>
      <div class="modal-footer">
        <button class="modal-btn favorite-btn">💖 Add to Favorites</button>
        <button class="modal-btn share-btn">📤 Share Poem</button>
      </div>
    </div>
  `
    ...
    // Add functionality to favorite and share buttons only
    ...
    shareBtn.addEventListener("click", (e) => {
      ...
      e.stopPropagation()
      this.playKidSound("click")
      this.showNotification("Sharing poem! 📤", "info")
    })
  }
  ...
  openPoemModal(card) {
    ...
    const fullPoem = card.querySelector(".poem-full")
    ...
    if (modal) {
      ...
      if (fullPoem) {
        modal.querySelector(".modal-poem-content").innerHTML = fullPoem.innerHTML
      } else {
        modal.querySelector(".modal-poem-content").innerHTML = card.querySelector(".poem-preview").innerHTML
      }
  ...
  setupPoemSearch() {
    ...
    searchInput.addEventListener("input", (e) => {
      clearTimeout(searchTimeout)
      searchTimeout = setTimeout(() => {
        this.performPoemSearch(e.target.value)
      }, 300)
    })
  }

  performPoemSearch(searchTerm) {
    const poemCards = document.querySelectorAll(".poem-card")
    const normalizedSearch = searchTerm.toLowerCase().trim()
    ...
    if (normalizedSearch) {
      this.showNotification(`Found ${visibleCount} poems matching "${searchTerm}" 📝`, "info")
    }
  }
  ...
  setupPoemFilters() {
    ...
    filterButtons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        this.filterPoemsByCategory(category)
        this.playKidSound("click")
      })
    })
  }
  ...
  filterPoemsByCategory(category) {
    ...
    const categoryName = category === "all" ? "all poems" : category
    this.showNotification(`Showing ${visibleCount} ${categoryName} poems 📖`, "success")
  }
